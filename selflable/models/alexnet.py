...
class AlexNet(nn.Module):
    def __init__(self, features, num_classes, input_size, init=True):
        super(AlexNet, self).__init__()
        

        self.adapt_layer = nn.Linear(input_size, 9216)
    ...
    def forward(self, x):
        ...
        if self.headcount == 1:
            ...
            for i in range(self.headcount):
                outputs.append(getattr(self, "top_layer%d" % i)(x))
            return outputs
...
def alexnet(bn=True, num_classes=[1000], init=True, size='big', input_size=1500):
    dim = 3
    model = AlexNet(make_layers_features(CFG[size], dim, bn=bn), num_classes, input_size, init)
    return model
