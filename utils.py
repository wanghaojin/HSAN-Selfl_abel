...
from selflable.models.alexnet import AlexNet
import torch.nn.functional as F
from sklearn.decomposition import PCA
from sklearn.metrics import adjusted_rand_score as ari_score
from sklearn.metrics.cluster import normalized_mutual_info_score as nmi_score
from selflable.self_label_model import ModelCluster
from selflable.eval_linear_probes import Probes
...
def phi(feature, true_labels, cluster_num):
    ...
    trunk = AlexNet(feature,[cluster_num],shape[1])
    probe_layers = [2, 5, 8, 10, 12]
    model = Probes(trunk,probe_layers,cluster_num,shape[1])
    ...
    # dataset = TensorDataset(feature,true_labels)
    # dataloader = DataLoader(dataset,batch_size=64,shuffle=True)
    # lr = 0.0005
    # lrdrop =150
    # o = ModelCluster(model,1,cluster_num,dataloader,num_epochs,lr,lrdrop,weight_decay=1e-5, batch_size=64, use_cpu=False)
    predict_labels, centers = kmeans(X=feature, num_clusters=cluster_num, distance="euclidean", device="cuda")
    # predict_labels,centers = o.predict()
    acc, nmi, ari, f1 = eva(true_labels, predict_labels, show_details=False)
    
    
    return 100 * acc, 100 * nmi, 100 * ari, 100 * f1, predict_labels, centers
...
def square_euclid_distance(Z, center):
    ...
    ZZ_CC = ZZ + CC
    ZZ_CC = ZZ_CC.reshape(2708,7)
    # print('zzcc',ZZ_CC.size())
    print('z',Z.shape)
